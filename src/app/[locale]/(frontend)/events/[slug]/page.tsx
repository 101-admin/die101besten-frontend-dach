...
import { DEFAULT_EDITION, DEFAULT_LANGUAGE, DEFAULT_SEO, Event } from "@/lib";
...
export async function generateStaticParams() {
  ...
  return (
    data?.map((event: Event) => ({
      slug: stripLocaleFromSlug(event.slug as string) || "#",
    })) || []
  );
}
...
export async function generateMetadata({
  ...
  const event = await EventsApi.getEventsBySlug(slug, locale);
...
const Page = async ({ params }: PageProps) => {
  ...
  const event = isEnabled
    ? await EventsApi.getEventsBySlugPreview(slug, locale)
    : await EventsApi.getEventsBySlug(slug, locale);
